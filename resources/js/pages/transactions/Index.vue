    ...
    <AppLayout :breadcrumbs="breadcrumbItems">
                ...
                <Table>
                    ...
                    <TableBody>
                        <template v-if="transactions.data.length > 0">
                            <TableRow v-for="transaction in transactions.data" :key="transaction.id">
                                ...
                                <TableCell>
                                    <span v-if="transaction.category" class="inline-flex items-center gap-2">
                                        <span class="flex h-6 w-6 items-center justify-center rounded-full" :style="{ backgroundColor: transaction.category.color }">
                         ...
                         <template v-else>
                            <TableRow>
                                <TableCell colspan="6" class="py-8 text-center text-gray-500 dark:text-gray-400">
        ...
        <Dialog :open="isAddEditModalOpen" @update:open="isAddEditModalOpen = $event">
             <DialogContent class="sm:max-w-lg" @close-auto-focus="closeModal">
                <DialogHeader>
                    <DialogTitle>{{ form.id ? 'Edit Transaction' : (form.type === 'expense' ? 'New Expense' : 'New Income') }}</DialogTitle>
                </DialogHeader>
