    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 p-4">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <!-- Date Selector -->
                <div
                    class="flex items-center gap-2 rounded-lg border border-white/10 bg-white/5 p-1 dark:bg-neutral-900">
                        ...
                        <PopoverContent class="w-auto border-neutral-800 bg-neutral-900 p-0 text-white" align="start">
                            <div class="w-72 rounded-lg p-4 shadow-lg">
                                ...
                                <div class="mt-4 flex justify-between border-t border-white/10 pt-3">
                                    ...
                                    <Button variant="ghost" @click="goToCurrentMonth" class="hover:bg-white/10">Current
                                        Month</Button>
                                </div>
                            </div>
                        </PopoverContent>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                ...
                <div class="rounded-xl border border-white/10 bg-white p-4 shadow-sm dark:bg-neutral-900">
                    <Link :href="route('transactions.incomes.index')" class="no-underline">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm text-gray-500 dark:text-gray-400">Incomes</p>
                                <p class="text-2xl font-bold text-emerald-500">{{ formatCurrency(stats.monthly_incomes,
                                    user.currency) }}</p>
                            </div>
                            <div
                                class="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-100 text-emerald-600 dark:bg-emerald-900 dark:text-emerald-300">
                                <Icon name="arrow-up" />
                            </div>
                        </div>
                    </Link>
                ...
                <div class="rounded-xl border border-white/10 bg-white p-4 shadow-sm dark:bg-neutral-900">
                    <Link :href="route('transactions.expenses.index')" class="no-underline">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-sm text-gray-500 dark:text-gray-400">Expenses</p>
                            <p class="text-2xl font-bold text-red-500">{{ formatCurrency(stats.monthly_expenses,
                                user.currency) }}</p>
                        </div>
                        <div
                            class="flex h-10 w-10 items-center justify-center rounded-full bg-red-100 text-red-600 dark:bg-red-900 dark:text-red-300">
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-5">
                <div class="lg:col-span-3 flex flex-col gap-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        ...
                        <div
                            class="rounded-xl border border-white/10 bg-white p-6 shadow-sm dark:bg-neutral-900 flex flex-col">
                            <h3 class="font-semibold text-lg mb-4">Expenses by category</h3>
                            <div v-if="expensesByCategory.length === 0"
                                class="flex flex-col items-center justify-center text-center text-gray-500 dark:text-gray-400 flex-grow">
                            ...
                            <div v-else class="flex-grow">
                                <div class="flex items-center justify-center h-40">
                                    <div class="relative w-36 h-36">
                                        <svg class="w-full h-full" viewBox="0 0 36 36"
                                            ...
                                            <circle cx="18" cy="18" r="15.9155" class="stroke-current text-red-500/10"
                                                fill="transparent" stroke-width="4"></circle>
                                            <circle v-for="(segment, index) in expenseDonutSegments" :key="index"
                                                cx="18" cy="18" r="15.9155" fill="transparent" stroke-width="4"
                                                :stroke="segment.color" stroke-dasharray="100"
                                                ...
                                                style="transform-origin: 50% 50%;"></circle>
                                        </svg>
                                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                                            <span class="text-xl font-bold text-red-400">{{ formatCurrency(totalExpense,
                                                user.currency) }}</span>
                                ...
                                <div class="mt-4 space-y-2 max-h-32 overflow-y-auto pr-2">
                                    <div v-for="category in expensesByCategory" :key="category.name"
                                        ...
                                        <span class="font-medium whitespace-nowrap">{{ formatCurrency(category.total,
                                            user.currency) }}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div
                            ...
                            <div v-if="incomesByCategory.length === 0"
                                class="flex flex-col items-center justify-center text-center text-gray-500 dark:text-gray-400 flex-grow">
                                    ...
                                    <div class="relative w-36 h-36">
                                        <svg class="w-full h-full" viewBox="0 0 36 36"
                                            ...
                                            <circle cx="18" cy="18" r="15.9155"
                                                class="stroke-current text-violet-500/10" fill="transparent"
                                                stroke-width="4"></circle>
                                            <circle v-for="(segment, index) in incomeDonutSegments" :key="index" cx="18"
                                                cy="18" r="15.9155" fill="transparent" stroke-width="4"
                                                :stroke="segment.color" :stroke-dasharray="`${segment.percentage} 100`"
                                        ...
                                        <div class="absolute inset-0 flex flex-col items-center justify-center">
                                            <span class="text-xl font-bold text-violet-400">{{
                                                formatCurrency(totalIncome, user.currency) }}</span>
                                            <span class="text-sm text-gray-400">Total</span>
                                        </div>
                                    </div>
                                </div>
                                <div class="mt-4 space-y-2 max-h-32 overflow-y-auto pr-2">
                                    <div v-for="category in incomesByCategory" :key="category.name"
                                        class="flex items-center justify-between text-sm">
                                        <div class="flex items-center gap-2 truncate">
                                            <span class="h-2 w-2 rounded-full flex-shrink-0"
                                                :style="{ backgroundColor: category.color }"></span>
                    ...
                    <div class="rounded-xl border border-white/10 bg-white p-6 shadow-sm dark:bg-neutral-900">
                        ...
                        <div v-if="!incomesVsExpenses || incomesVsExpenses.every(d => d.incomes === 0 && d.expenses === 0)"
                            class="flex flex-col items-center justify-center text-center text-gray-500 dark:text-gray-400 h-48">
                        ...
                        <div v-else class="h-48 flex justify-around items-end gap-2">
                            <div v-for="(monthData, index) in incomesVsExpenses" :key="index"
                                class="flex flex-col items-center w-full h-full justify-end">
                                <div class="flex gap-1 items-end h-full w-full justify-center">
                                    <div class="w-1/3 bg-emerald-500 rounded-t-sm"
                                        :style="{ height: `${(monthData.incomes / maxIncomesVsExpenses) * 100}%` }">
                                    </div>
                                    <div class="w-1/3 bg-red-500 rounded-t-sm"
                                        :style="{ height: `${(monthData.expenses / maxIncomesVsExpenses) * 100}%` }">
                                    </div>
                                </div>
                    ...
                    <div class="rounded-xl border border-white/10 bg-white p-6 shadow-sm dark:bg-neutral-900">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <Button variant="outline" size="sm"
                                class="dark:bg-violet-500/20 dark:border-violet-500/40 dark:text-violet-300">Last 7
                                days</Button>
                        </div>
                        <div v-if="!expenseFrequency || expenseFrequency.every(d => d.total === 0)"
                            class="flex flex-col items-center justify-center text-center text-gray-500 dark:text-gray-400 h-32">
                        ...
                        <div v-else class="flex flex-col justify-between h-40">
                            <svg class="w-full h-full" viewBox="0 0 280 100" preserveAspectRatio="none">
                                <polyline :points="expenseFrequencyPoints" fill="rgba(139, 92, 246, 0.2)"
                                    stroke="#8B5CF6" stroke-width="2" />
                            </svg>
                            <div class="flex justify-between text-xs text-gray-400 mt-2">
                                <span v-for="item in expenseFrequency" :key="item.date" class="w-full text-center">{{
                                    item.date }}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center gap-4">
                                <div class="relative h-16 w-4 bg-red-500/20 rounded-full overflow-hidden">
                                    <div class="absolute bottom-0 left-0 w-full bg-emerald-500"
                                        :style="{ height: `${balanceBarPercentage}%` }"></div>
                                </div>
                                <div class="flex-1 space-y-2">
                                    <div class="flex justify-between items-center text-sm">
                                        <span class="text-gray-400">Incomes</span>
                                        <span class="font-semibold text-emerald-400">{{
                                            formatCurrency(stats.monthly_incomes, user.currency) }}</span>
                                    </div>
                                    <div class="flex justify-between items-center text-sm">
                                        <span class="text-gray-400">Expenses</span>
                                        <span class="font-semibold text-red-400">{{
                                            formatCurrency(stats.monthly_expenses, user.currency) }}</span>
                                    </div>
                                    <div
                                        class="flex justify-between items-center text-sm pt-2 border-t border-white/10">
                                        <span class="text-gray-400">Balance</span>
                                        <span class="font-semibold">{{ formatCurrency(monthlyBalance, user.currency)
                                            }}</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="rounded-xl border border-white/10 bg-white p-6 shadow-sm dark:bg-neutral-900">
                        ...
                        <div class="space-y-2">
                            <div class="flex justify-between items-center text-sm">
                                ...
                                <span class="font-semibold text-red-400">{{
                                    formatCurrency(stats.outstanding_expenses_total, user.currency) }}</span>
                            </div>
                            <div class="flex justify-between items-center text-sm">
                                <span class="text-gray-400">Outstanding incomes total</span>
                                <span class="font-semibold text-emerald-400">{{
                                    formatCurrency(stats.outstanding_incomes_total, user.currency) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
